...
// @ FEEDBACK TailwindCSS Main Plugin
    ...
    }) =>
    {
        ...
        addComponents(
            _.fromPairs(_.map(options.themes, (theme, themeName) => [
                ...
                ))), (value, key) => [[`--feedback-${e(key)}`, value], [`--feedback-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
            ]))
        );
        ...
        const schemeCustomProps = _.map(['light', 'dark'], (colorScheme) =>
        {
            ...
            const background = theme(`feedback.customProps.background.${colorScheme}`);
            const foreground = theme(`feedback.customProps.foreground.${colorScheme}`);
            ...
            return {
                [(isDark ? darkSchemeSelectors : lightSchemeSelectors)]: {
                    ...
                    ..._.fromPairs(_.flatten(_.map(background, (value, key) => [[`--feedback-${e(key)}`, value], [`--feedback-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
                    ..._.fromPairs(_.flatten(_.map(foreground, (value, key) => [[`--feedback-${e(key)}`, value], [`--feedback-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
                }
            };
        });
            ...
            theme: {
                extend: {
                    ...
                    colors: _.fromPairs(_.flatten(_.map(_.keys(flattenColorPalette(normalizeTheme(options.themes.default))), (name) => [
                        [name, `rgba(var(--feedback-${name}-rgb), <alpha-value>)`],
                        [`on-${name}`, `rgba(var(--feedback-on-${name}-rgb), <alpha-value>)`]
                    ])))
                },
                feedback  : {
                    customProps: {
