...
import { ScrollbarGeometry, ScrollbarPosition } from '@feedback/directives/scrollbar/scrollbar.types';

/**
 * Wrapper directive for the Perfect Scrollbar: https://github.com/mdbootstrap/perfect-scrollbar
 */
@Directive({
    selector: '[feedbackScrollbar]',
    exportAs: 'feedbackScrollbar'
})
export class FeedbackScrollbarDirective implements OnChanges, OnInit, OnDestroy {
    static ngAcceptInputType_feedbackScrollbar: BooleanInput;

    @Input() feedbackScrollbar: boolean = true;
    @Input() feedbackScrollbarOptions: PerfectScrollbar.Options;
    ...
    ngOnChanges(changes: SimpleChanges): void {
        if ( 'feedbackScrollbar' in changes ) {
            this.feedbackScrollbar = coerceBooleanProperty(changes.feedbackScrollbar.currentValue);
            if ( this.feedbackScrollbar ) {
                this._init();
            } else {
                this._destroy();
            }
        ...
        if ( 'feedbackScrollbarOptions' in changes ) {
            this._options = merge({}, this._options, changes.feedbackScrollbarOptions.currentValue);
            if ( !this._ps ) {
                return;
            }
